function [Path, distanceX, OPEN_num, open, run_time, debugInfo] = IAstar_FiveDir_Fallback_Fast(GridMap, St, Ta, diagRule)
% Computationally optimized copy of the final IA* global search.
%
...
% This Fast version preserves the algorithm and accelerates repeated
% calculations using heuristic maps, weight maps, and a summed-area table
% for the obstacle density rectangle.
%
% Inputs:
...
obsIntegral = build_obstacle_integral(GridMap);

[rowGrid, colGrid] = ndgrid(1:MaxRow, 1:MaxCol);
hMap = sqrt((rowGrid - xTarget).^2 + (colGrid - yTarget).^2);
goal_distance = hMap(xStart, yStart);
if goal_distance > 0
    weightMap = 1 + hMap / goal_distance;
else
    weightMap = ones(MaxRow, MaxCol);
end
...
while ((xNode ~= xTarget || yNode ~= yTarget) && NoPath == 1)
    ...
    [exp_array, five_count] = expand_array_fivedir_fast( ...
        ...
        MaxCol, MaxRow, diagRule, hMap, weightMap, obsIntegral);
...
function [exp_array, five_count] = expand_array_fivedir_fast(node_x, node_y, gn, xTarget, yTarget, closed_flag, GridMap, MaxCol, MaxRow, diagRule, hMap, weightMap, obsIntegral)
    ...
    for dirIdx = 1:size(candidate_dirs, 1)
        ...
        exp_array(five_count,:) = make_candidate(node_x, node_y, s_x, s_y, gn, ...
            xTarget, yTarget, hMap, weightMap, obsIntegral);
    end
...
function [exp_array, eight_count] = expand_array_eightdir_fallback(node_x, node_y, gn, xTarget, yTarget, closed_flag, GridMap, MaxCol, MaxRow, diagRule, hMap, weightMap, obsIntegral)
    exp_array = zeros(8,5);
    eight_count = 0;
...
function candidate = make_candidate(node_x, node_y, s_x, s_y, gn, xTarget, yTarget, hMap, weightMap, obsIntegral)
    h_val = hMap(s_x, s_y);
    weight = weightMap(s_x, s_y);
    obsdensity = obs_density_fast(s_x, s_y, xTarget, yTarget, obsIntegral);
    if node_x ~= s_x && node_y ~= s_y
        step_cost = 1.4142135623730951;
    else
        step_cost = 1;
    end
...
function [Path, distanceX] = reconstruct_path(xNode, yNode, xStart, yStart, xTarget, yTarget, OPEN, open_index_map, MaxRow, MaxCol, GridMap)
    ...
    Path = Floyd_Smooth_safer(Path, GridMap, 0.25);
...
function obsIntegral = build_obstacle_integral(GridMap)
    obsIntegral = zeros(size(GridMap, 1) + 1, size(GridMap, 2) + 1);
    obsIntegral(2:end, 2:end) = cumsum(cumsum(double(GridMap), 1), 2);
end

function P_obs = obs_density_fast(node_x, node_y, xTarget, yTarget, obsIntegral)
    x_min = min(node_x, xTarget);
    x_max = max(node_x, xTarget);
    y_min = min(node_y, yTarget);
    y_max = max(node_y, yTarget);

    obs_count = obsIntegral(x_max + 1, y_max + 1) ...
        - obsIntegral(x_min, y_max + 1) ...
        - obsIntegral(x_max + 1, y_min) ...
        + obsIntegral(x_min, y_min);
    total_grids = (x_max - x_min + 1) * (y_max - y_min + 1);
    P_obs = obs_count / total_grids;
end
